...
function applyRule(rule){state.angle=rule.angle;state.strength=rule.strength;$("#force").value=rule.strength;$("#forceOutput").textContent=rule.strength+"%";$("#forceReadout").textContent=rule.strength+"%";$("#fieldReadout").textContent=rule.strength===0?"0G":rule.angle+"°";$("#directionGlyph").textContent=rule.glyph;$("#fieldMessage").textContent=rule.message;$("#ruleName").textContent=rule.name.toUpperCase();$("#ruleSql").textContent=rule.sql;$$('[data-direction]').forEach(button=>button.classList.toggle("active",Number(button.dataset.direction)===rule.angle&&rule.strength>0));$("#microgravity").classList.toggle("active",rule.strength===0);$$('.rule-card').forEach(card=>card.classList.toggle("active",card.dataset.rule===rule.slug));const radians=rule.angle*Math.PI/180,distance=rule.strength===0?0:86,demo=$("#ruleDemonstrator");demo?.style.setProperty("--test-x",`${-Math.cos(radians)*distance}px`);demo?.style.setProperty("--test-y",`${Math.sin(radians)*distance}px`);demo?.style.setProperty("--test-rotate",`${rule.angle-90}deg`);if($("#cabinetRuleName"))$("#cabinetRuleName").textContent=rule.name;if($("#cabinetRuleMessage"))$("#cabinetRuleMessage").textContent=rule.message;if($("#cabinetVector"))$("#cabinetVector").textContent=rule.glyph;chord([170,230,310])}
